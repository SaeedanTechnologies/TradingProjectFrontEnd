...
const TradingAccountModal = ({ setIsModalOpen, fetchData, TradingGroupID }) => {
  ...
  const fetchDataWRTID = async () => {
    ...
    if (success) {
      ...
      const selectedOptions = GroupList.filter(option => {
        return payload.symbel_group.some(symbol => symbol.pivot.symbel_group_id === option.id);
      });
  ...
  const handleSumbit = async () => {
    try {
      ...
      const TradingGroupData = {
        ...
        mass_swap: massSwap,
        symbel_group_id: SelectedGroup.map(item => item.id)
      }
